...
public class Registers {
    ...
    public String getBinaryStringIr() {
        try {
            // Assuming this.ir is actually an octal number incorrectly stored as decimal
            int actualDecimalValue = Integer.parseInt(String.valueOf(this.ir), 8);
            //System.out.println("Decimal Value (corrected): " + actualDecimalValue);

            // Convert the corrected decimal integer to a binary string, formatted to 16 bits
            return String.format("%16s", Integer.toBinaryString(actualDecimalValue)).replace(" ", "0");
    ...
    public String getOctalStringIr() {
        if (this.ir <= 0xffff) {
            return String.format("%6s", Integer.toBinaryString(this.ir)).replace(" ", "0");
        }
        return null;
    }
    ...
    public void increasePCByOne() {
        if (FrontPanel.instructionIndex < FrontPanel.addressList.size()) {
            this.pc = FrontPanel.addressList.get(FrontPanel.instructionIndex);
            FrontPanel.instructionIndex++;
        }
    }
